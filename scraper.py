...
SOURCES = {
    ...
    "novelbin.com": {"title": "h3.title", "cover": ".book img", "author": "div.info a[href*='author']", "chapter_content": "div#chr-content"},
    "novelbin.me": {"title": "h3.title", "cover": ".book img", "author": "div.info a[href*='author']", "chapter_content": "div#chr-content"},
...
class NovelScraper:
    ...
    async def _get_chapters(self, soup):
        if "royalroad.com" in self.domain:
            ...
            return chapters
        if "novelbin" in self.domain:
            return await self._novelbin_chapters()
        if "novelfull.com" in self.domain:
            return await self._paginated_chapters(soup)
        return await self._smart_chapters(soup)

    async def _novelbin_chapters(self):
        """Fetch all chapters from novelbin AJAX endpoint — gets full list instantly."""
        try:
            # Extract novel slug from URL e.g. /novel-book/deep-sea-embers -> deep-sea-embers
            slug = self.url.rstrip("/").split("/")[-1]
            # Remove any hash fragments
            slug = slug.split("#")[0]
            base = f"https://{self.domain}"
            ajax_url = f"{base}/ajax/chapter-archive?novelId={slug}"
            s = await self._sess()
            ajax_headers = {**HEADERS, "X-Requested-With": "XMLHttpRequest", "Referer": self.url}
            async with s.get(ajax_url, headers=ajax_headers, timeout=aiohttp.ClientTimeout(total=30)) as r:
                if r.status == 200:
                    html = await r.text()
                    ...
                    for a in soup.find_all("a", href=True):
                        ...
                        href = a["href"]
                        if title and href:
                            chapters.append({"title": title, "url": urljoin(base, href)})
                    if chapters:
                        print(f"[scraper] novelbin AJAX: {len(chapters)} chapters found")
                        return chapters
        except Exception as e:
            print(f"[scraper] novelbin AJAX failed: {e}")
        # Fallback to paginated scraping
        soup = await self.fetch(self.url)
        return await self._paginated_chapters(soup)
